...
export const useAdvancedFinancialEngine = () => {
  ...
  const calculateAdvancedInsight = useCallback(async (
    ...
    try {
      ...
      const moodData = MOOD_ARCHETYPES[data.mood as keyof typeof MOOD_ARCHETYPES] || MOOD_ARCHETYPES[5];
      ...
      // Enhanced hidden costs detection
  ...
  const testAIConnection = useCallback(async () => {
    return { success: true, provider: aiProvider };
  }, [aiProvider]);
